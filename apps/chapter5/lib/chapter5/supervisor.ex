defmodule Chapter5.Supervisor do
  use GenServer
  ...
  def terminate_child(supervisor, pid) when is_pid(pid) do
    GenServer.call(supervisor, {:teminate_child, pid})
  end
  ...
  def handle_call({:start_child, child_spec}, _from, state) do
    IO.puts("Starting child: #{inspect(child_spec)}")
  ...
  def handle_call({:terminate_child, pid}, _from, state) do
    case terminate_child(pid) do
      ...
      :error ->
        {:reply, {:error, "error terminatint child"}, state}
    end
  end
  ...
  def handle_call(:count_children, _from, state), do: {:reply, Enum.count(state), state}
  ...
  defp start_children(child_specs) do
    do_start_children(child_specs, [])
  end

  defp do_start_children([], child_list), do: child_list

  defp do_start_children([child_spec | t], child_list) do
    case start_child(child_spec) do
      {:ok, pid} ->
        do_start_children(t, [{pid, child_spec} | child_list])
